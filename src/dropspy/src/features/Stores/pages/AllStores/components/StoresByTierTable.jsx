...
const StoresByTierTable = (props) => {

  const TABLE_PROPORTION = 130;
  const TABLE_MOBILE_VISIBLE = ['tier', 'totalStores', 'totalProducts', 'totalRevenue']

  const { tierStores } = props.value
  const TABLE_COMPLETE_FIELDS_ARR = getOnlyFoundFields(tierStores[0])
  ...
  return (
    <>
      ...
      <Table value={{ columns: visibleColumns, data: tierStores }} />
    </>
  );

};

export { StoresByTierTable }
